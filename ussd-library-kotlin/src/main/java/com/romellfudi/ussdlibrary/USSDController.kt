...
object USSDController : USSDInterface, USSDApi {
    ...
    override fun callUSSDInvoke(baseContext: Context, ussdPhoneNumber: String, hashMap: HashMap<String, List<String>>,
        ...
        context = baseContext
        callUSSDInvoke(context, ussdPhoneNumber, 1, hashMap, callbackInvoke)
    }
    ...
    override fun callUSSDOverlayInvoke(baseContext: Context, ussdPhoneNumber: String, hashMap: HashMap<String, List<String>>,
                                       callbackInvoke: CallbackInvoke) {
        context = baseContext        
        callUSSDOverlayInvoke(context, ussdPhoneNumber, 1, hashMap, callbackInvoke)
    }
    ...
    override fun verifyAccessibilityAccess(context: Context): Boolean =
            isAccessibilityServicesEnable(context).also {
                if (!it) openSettingsAccessibility(context)
            }
    ...
    private fun openSettingsAccessibility(context: Context) =
            with(AlertDialog.Builder(context)) {
                setTitle("USSD Accessibility permission")
                setMessage("You must enable accessibility permissions for the app %s".format(getNameApp(context)))
                setCancelable(true)
    ...
    private fun getNameApp(context: Context): String = when (context.applicationInfo.labelRes) {
        0 -> context.applicationInfo.nonLocalizedLabel.toString()
        else -> context.getString(context.applicationInfo.labelRes)
    }
